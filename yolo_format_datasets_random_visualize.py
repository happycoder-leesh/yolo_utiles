...
def main():
    ...
    selected_image_paths = random.sample(image_paths, int(len(image_paths) * args.ratio))

    for img_path in selected_image_paths:
        file_name = os.path.basename(img_path)
        label_path = os.path.join(args.label_dir, file_name.replace('.jpg', '.txt'))
    ...
    print(f"[INFO] {int(args.ratio * 100)}%로 선택된 이미지에 대해 시각화된 이미지가 {output_dir} 폴더에 저장되었습니다.")
